...
impl Kaleidoscope for BitRepresentation {
    ...
    fn possible(&self, piece_idx: usize, reverse: bool) -> Vec<Self::Move> {
...
impl Kaleidoscope for VectorRepresentation {
    ...
    fn play(&mut self, mv: &Self::Move) {
        let piece = &self.pieces[mv.piece_idx].configs[mv.config_idx];
        let dim_1 = usize::from(piece.width);
        let dim_2 = usize::from(piece.height);

        for x in 0..dim_1 {
            for y in 0..dim_2 {
                let color = self.pieces[mv.piece_idx].get_piece_color(mv.config_idx, x, y);
                if color != 0 {
                    // only place non-empty cells
                    let idx = self.board_idx(mv.col + x, mv.row + y);
                    self.board[idx] = Some(u8::try_from(mv.piece_idx).unwrap());
                }
            }
        }
    }
