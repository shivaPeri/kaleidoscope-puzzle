...
impl<T: Kaleidoscope + std::marker::Sync + std::marker::Send> Solver<T> for MultithreadedSolver<T> {
    type Strategy = [usize; 18];

    fn solve(&mut self, strategy: Self::Strategy) -> bool {
        let thread1 = thread::spawn(move || {
            let game1 = &self.game.rotate90(0);
            let mut solver = BacktrackingSolver::<T>::new(*game1);
            return solver.solve(strategy);
        });
